...
export class WeatherService implements OnModuleInit {
  ...
  async getSevenDayForecast(lat: number, lon: number) {
    const apiKey = this.configService.get<string>('AEMET_API_KEY');
    if (!apiKey) {
    ...
    const key = this.cache.key('weather', 'forecast', municipioCode);
    const ttlMs = this.configService.get<number>(
      'CACHE_TTL_WEATHER_MS',
      1800000,
    );
  ...
  private transformAemetData(rawData: any) {
    ...
    return dias.map((diaInfo: any) => {
      ...
      const maxLluviaArr = diaInfo.probPrecipitacion.map((p: any) =>
      ...
      const cieloRepresentativo =
        diaInfo.estadoCielo.find((c: any) => c.value !== '')?.value || '11';
      const descCielo =
        diaInfo.estadoCielo.find((c: any) => c.descripcion !== '')
          ?.descripcion || 'Despejado';

      // Viento (cogemos la velocidad máxima del día)
      const maxVientoArr = diaInfo.viento?.map((v: any) =>
        v.velocidad !== '' ? Number(v.velocidad) : 0,
      ) || [0];
